...
- 23 个 provider module 已自动发现；其中 22 个提供原生 inventory 或受约束 positive evidence，仅 Talemetry 提供 detection-only typed incomplete semantics。Meta 仍只接受 visible-page positive evidence；CEIPAL 已冻结并验证 public inventory schema，Talemetry 仍需完成同等 contract 才能升级为可支持 candidate/no-match 的 adapter。
...
- `.57` 将 evidence strength 与执行预算绑定：只有 scheduler tier 0-2 的未尝试候选构成 retryable career fetch exhaustion，tier 3 speculative truncation 是确定性 miss。S5 direct provider handoff 只接受官网可见链接、registry listing-capable adapter、非 detail URL 且输入 URL 已是 provider canonical board root；legacy redirect 和 detail URL 继续 fetch/adapter 验证。S6 对 native adapter 的完整 verified inventory 使用 terminal no-match，不再进入 generic HTML fallback；incomplete/unsupported provider 仍允许 fallback。该边界使 Percepta 越过 generic cap 到 Taleo，同时不绕过 Paycom/Lever 既有 contract，并把 Akkodis 当前 9 页 Sitecore no-match 从 hard timeout 缩短为 36.7 秒结构化完成。最终离线门禁为 871 tests、24/24 provider、6/6 resolver、23 adapters / 0 issues；冻结 30-company 为 30/28/26/23。
