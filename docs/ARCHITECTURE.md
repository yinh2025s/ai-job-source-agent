...
- S2 的请求时间分配不改变候选 authority。验证 allocator 已经选中的第一个 exact core-name `.com` apex 可以使用一个较大的有界单次 `identity_apex_candidate` 窗口，避免真实官网与普通机械猜测共用两秒 speculative slice；同一记录的其他候选、前后缀域名、非 `.com` 和不相关域名继续使用短 `speculative_candidate` policy。该窗口不增加 score、不允许额外 retry，也不能绕过 homepage 内容、canonical、Organization、regional 或 company identity 门。
