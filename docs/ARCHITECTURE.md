...
- `.49` 当前门禁为 CPython 3.12 749 tests、23/23 provider、6/6 resolver 和 architecture validation 23 adapters / 0 issues。相同 frozen-30 manifest 的 live 结果为 30/28/26/20，10 个 failure bundle 全部成功；Direct Supply 的旧公开 career/Workday route 当前不可达，Finch 的旧 exact listing 已从当前 Ashby inventory 下线，因此不把外部证据漂移伪装成代码提升或回归。Akkodis 可识别 Sitecore/Next official job list 并完整检查 85 条 filtered inventory；unpacked extension 已安装，真实登录态 LinkedIn Scan/Run 继续 deferred。
