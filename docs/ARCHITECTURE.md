...
- ADR-0009 将 outer live budget 与 domain outcome 分开：只有 `COMPANY_TIME_BUDGET_EXHAUSTED`、完整 authoritative upstream chain、replay 越过超时 stage、无 fixture gap 且 source identity prefix 不漂移时才输出 passing `budget_recovery`。ADR-0033 进一步冻结同阶段 normalization：scoped tape 全部消费后，只有同阶段非 retryable 终态、身份前缀不变且无 expected transition 时，才恢复录制的 company-budget terminal，并把底层 replay outcome 留在 trace。Later-stage advancement 仍属于 ADR-0009。Legacy/scoped bundle schema 当前为 `6`/`8`。Expected transition 不能绕过 URL/provider 检查。Snapshot body 的 unquoted sensitive key 使用 JavaScript identifier 左边界，standalone `code` 仍脱敏而 `urlCode` 不再损坏。旧过度脱敏 blob 不能猜测修复，必须重新 capture；Percepta 新 capture 的 9 fixtures 可 1/1 复现 Taleo HTTP 500。`.58` 门禁为 876 tests、24/24 provider、6/6 resolver、23 adapters / 0 issues。
...
- ADR-0023 / `.88` 在 replay orchestration 边界增加 pre-execution scope integrity gate：effective replay start 继续使用 authoritative checkpoint-prefix 规则，start 后必须存在连续 captured stage scope；缺少 active-stage boundary 时只发布原子 failed manifest，不创建 replay input/checkpoint，也不把 earlier scope 冒充 terminal evidence。Resolver 的 title identity 读取 bounded HTML head（最多 64 KiB），普通 token evidence 仍限前 5 KiB；S4 search 与 homepage link 共用 target-title audience mismatch，ATS portfolio 只新增一个 provider-neutral bounded query。Fresh frozen-15 为 14 website、12 career、12 verified job list、8 exact，full scoped replay 15/15；最终门禁为 1322 tests、25/25 provider、6/6 resolver、26 adapters / 0 issues。
...
- Correctness stabilization freezes a typed identity chain from hiring entity through provider, tenant, canonical board, and opening. S7 fails closed and clears rejected exact URLs. S6 requires the opening adapter to agree on provider/tenant and, when its board locator differs from the canonical S5 URL, requires selected-candidate evidence bound to the typed S5 board handoff; this survives checkpoint resume without accepting an untraced same-tenant board substitution. Availability requires `inventory.complete is True` before a verified no-match or no-public conclusion. Resolver and derived-provider retries retain typed fetch projections in trace. Evaluation annotations are merged only after source digest, cohort identity, URL, coverage, and schema validation; reviewed artifacts remain separate from the immutable live capture. The observed 30-company capture is 30/30 replay-stable and its review is documented in `docs/STABILIZATION_EVALUATION_REPORT.md`.
