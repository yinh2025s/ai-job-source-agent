...
Composition root 最外层使用每公司进程内的 bounded LRU page cache，只缓存成功、无 body、无 headers 的 GET response，使 S4/S5/S6 可以复用同一 landing page。Request URL、`Page.url` 与 redirect `final_url` 作为同一 entry 的别名一起命中和淘汰，因此下游按最终 URL 访问不会重新 transport。POST、带 headers 请求、失败响应和跨进程状态不进入 cache；cache 内容不写入 checkpoint、trace 或 snapshot，snapshot/retry 仍位于 cache 内层并保持各自 contract。
