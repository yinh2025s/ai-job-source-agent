...
class OpeningMatcherTests(unittest.TestCase):
    ...
    def test_full_title_fallback_wins_before_deferred_landing_js_assets(self):
        job_list_url = "https://careers.example.com/jobs"
        direct_url = job_list_url + "?q=Data+Analyst"
        asset_url = "https://careers.example.com/assets/jobs.js"
        opening_url = "https://careers.example.com/jobs/42/data-analyst"
        ...
        class RecordingFetcher:
            ...
            def fetch(self, url, data=None, headers=None):
                ...
                if url == job_list_url:
                    return Page(url, f'<script src="{asset_url}"></script>')
                if url == direct_url:
                    return Page(url, f'<a href="{opening_url}">Data Analyst</a>')
                if url == asset_url:
                    raise AssertionError("deferred landing JS asset should not be fetched")
                return Page(url, "<main>No matching inventory</main>")

        fetcher = RecordingFetcher()
        with patch(
            "job_source_agent.opening_matcher.probe_first_party_provider_assets",
            side_effect=lambda _fetcher, page, *_args: (page, None),
        ), patch(
            "job_source_agent.opening_matcher.resolve_declared_search_route",
            return_value=SimpleNamespace(
                status="not_declared", helper_url=None, route_url=None
            ),
        ):
            match, trace = JobOpeningMatcher(fetcher).match(job_list_url, "Data Analyst")

        self.assertIsNotNone(match)
        assert match is not None
        self.assertEqual(match.url, opening_url)
        self.assertEqual(fetcher.calls, [job_list_url, direct_url])
        self.assertNotIn("js_declared_inventory", trace)

    def test_deferred_landing_js_runs_after_nonmatching_direct_routes(self):
        job_list_url = "https://careers.example.com/jobs"
        direct_urls = [
            job_list_url + "?q=Data+Analyst",
            job_list_url + "?search=Data+Analyst",
            job_list_url + "?query=Data+Analyst",
        ]
        asset_url = "https://careers.example.com/assets/jobs.js"
        endpoint_url = "https://careers.example.com/bin/public/jobs"
        opening_url = "https://careers.example.com/jobs/42/data-analyst"
        ...
        class RecordingFetcher:
            ...
            def fetch(self, url, data=None, headers=None):
                ...
                if url == endpoint_url:
                    return Page(
                        url,
                        json.dumps({"jobPostings": [{"title": "Data Analyst", "url": opening_url}]}),
                    )
                return Page(url, "<main>No matching inventory</main>")
        ...
        assert match is not None
        self.assertEqual(match.url, opening_url)
        self.assertEqual(fetcher.calls[:4], [job_list_url, *direct_urls])
        self.assertEqual(fetcher.calls[4:6], [asset_url, endpoint_url])
        self.assertEqual(len(trace["js_declared_inventory"]), 4)
        self.assertEqual(
            [item["status"] for item in trace["js_declared_inventory"][:3]],
            ["transport_not_declared"] * 3,
        )
        self.assertEqual(trace["js_declared_inventory"][-1]["status"], "verified")

    def test_deferred_landing_js_rejects_wrong_location_and_cross_site_results(self):
        job_list_url = "https://careers.example.com/jobs"
        asset_url = "https://careers.example.com/assets/jobs.js"
        endpoint_url = "https://careers.example.com/bin/public/jobs"
                            ...
                            {"jobPostings": [
                                {
                                    "title": "Data Analyst",
                                    "location": "Toronto, ON",
                                    "url": "https://careers.example.com/jobs/42/data-analyst",
                                },
                                {
                                    "title": "Data Analyst",
                                    "location": "Austin, TX",
                                    "url": "https://other.example.com/jobs/43/data-analyst",
                                },
                            ]}
                        ),
                    )
