...
_CALL = re.compile(r"\bCWS\s*\.\s*jobs\s*\.\s*(set_api|set_options)\s*\(")
_OPTION = re.compile(
    r"(?:^|[,\{])\s*(?:['\"])?(org_id|jobdetail_path|limit)(?:['\"])?\s*:\s*"
    r"(?:(['\"])(.*?)\2|(\d+))",
    re.DOTALL,
)
...
class CWSAdapter:
    ...
    def identify_board_from_page(self, page: Page) -> JobBoard | None:
        ...
        if parsed_page is None or config is None:
            return None
        api_url, org_id, detail_path, limit = config
        board_url = urlunparse(parsed_page._replace(query="", fragment=""))
        identifier = _identifier(board_url, api_url, org_id, detail_path, limit)
        return JobBoard(
            url=board_url,
            provider=self.name,
    ...
    def list_jobs(self, fetcher, board: JobBoard, query: JobQuery) -> AdapterResult:
        ...
        board_url, api_url, org_id, detail_path, limit = identity
        ...
        for page_number in range(_MAX_PAGES):
            ...
            request_url = _request_url(endpoint, title, org_id, limit, offset)
            try:
            ...
            parsed = _inventory_page(payload, org_id)
        ...
        inventory_complete = failure_reason is None and expected_total == records_seen
        if not inventory_complete and failure_reason is None:
            failure_reason = "FETCH_BUDGET_EXHAUSTED"
            stop_reason = "page_cap_reached"
        reason_code = failure_reason
        if inventory_complete and not candidates:
        ...
        exposed_candidates = candidates if inventory_complete else []
...
def _page_config(html: str, parsed_page) -> tuple[str, str, str, int] | None:
    ...
    for raw_script in parser.scripts:
        ...
        for name, argument in _calls(script):
            ...
            if not argument.startswith("{") or not argument.endswith("}"):
                return None
            for match in _OPTION.finditer(argument):
                key = match.group(1)
    ...
    if len(api_urls) != 1 or len(org_ids) != 1 or len(detail_paths) != 1:
        return None
    return next(iter(api_urls)), next(iter(org_ids)), next(iter(detail_paths)), limit
...
def _board_identity(board: JobBoard) -> tuple[str, str, str, str, int] | None:
    ...
    try:
        ...
        return None
    if parsed_board is None or not isinstance(value, dict) or set(value) != {
    ...
    if (
        ...
        or board.identifier != _identifier(board.url, api_url, org_id, detail_path, limit)
    ):
        return None
    return board.url, api_url, org_id, detail_path, limit


def _request_url(endpoint: str, title: str, org_id: str, limit: int, offset: int) -> str:
...
def _inventory_page(payload, org_id: str) -> tuple[list[dict], int] | str:
    if not isinstance(payload, dict) or set(payload) - {
        "totalHits", "queryResult", "organization", "Organization", "org_id",
        "aggregations", "histogramResults", "location",
    }:
    ...
    if (
        ...
        return "invalid_response_schema"
    if not _org_continuity(payload, org_id):
