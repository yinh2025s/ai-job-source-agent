...
Current adapter iteration: `2026-07-14.71`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. Full-outcome replay fails closed on dropped records, and evaluation baselines bind the observed result count. S5 preserves retryable evidence-backed failures instead of reporting a deterministic missing board. The per-company page cache now aliases a successful GET by request, page, and redirect-final URL, so adjacent stages reuse one transport without persisting HTML or crossing a process boundary; resume preflight rejects semantically unusable checkpoint chains and rebuilds safely. Final offline gates pass 1004 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. The latest frozen live run remains 30 verified websites, 29 career pages, 27 verified job lists, and 23 exact openings. A focused Adobe live run now reaches its Workday board and exact Forward-Deployed AI Engineer opening with only one real request to the redirecting career root. Authenticated LinkedIn extension Scan/Run acceptance remains deferred while backend reliability work continues.
