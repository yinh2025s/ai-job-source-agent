...
Current adapter iteration: `2026-07-14.84`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. ADR-0017 adds a bounded `JobBoardPortfolio`: S5 expands a scoped early-career/university board only through native-adapter verification, S6 uses a versioned attempt cap, and company-wide empty/no-match conclusions require a complete eligible set. Single-board behavior remains compatible. Workday and SmartRecruiters public locators can be checkpointed only under strict host/path/identifier policies; no provider payload or browser state is persisted. A focused Visa capture reached its official general Workday board and exact `Sr Data Scientist` opening, and exposed a pre-existing website-only handoff contradiction; S2 rejection now clears that unverified URL before downstream stages, while explicit career-root/external-apply handoffs retain their own evidence path. Final offline gates pass 1249 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 issues. The prior strict 15-company baseline remains 6/15 pre-frozen exact opening identities until the same full cohort is rerun. Hostinger dynamic inventory is the next backend contract. Authenticated LinkedIn extension Scan/Run remains a manual acceptance gate; the user performs the Chrome clicks and the backend validates bridge artifacts and logs.
