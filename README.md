...
Current adapter iteration: `2026-07-14.66`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. ADR-0005/0006 define typed board handoff and request-aware snapshot replay; later ADRs bind deterministic configuration, durable publication, audited budget recovery, and cooperative provider budgets. Replay now restores verified response URL/final-URL identity, preserves typed fixture gaps through S4/S5, and compares provider-declared canonical board identity without weakening provider, tenant, or exact-opening checks. Final gates pass 936 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. The latest frozen 30-company run remains 30 verified websites, 29 career pages, 28 verified job lists, and 24 exact openings. A fresh Product Manager public-search sample produced 17 unique companies with 17 websites, 10 career/job-list pages, and 7 exact openings; its snapshot replay reached 16 reproduced, one explicit Adobe capture gap, and zero mismatches. Authenticated LinkedIn extension Scan/Run acceptance is deferred while backend reliability work continues.
