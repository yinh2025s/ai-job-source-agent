...
Current adapter iteration: `2026-07-14.69`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. Full-outcome replay now fails closed on dropped records, and evaluation baselines bind the observed result count. S5 preserves retryable evidence-backed failures instead of reporting a deterministic missing board; Phenom can derive a customer-owned search board only from validated tenant/CDN/base-route state. Career search and Sitecore pagination stop cooperatively before publication time is consumed, native budget interruption does not trigger generic fallback, and parent timeout recovery publishes durable-stage provenance. Final offline gates pass 999 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. The `.68` frozen live run produced 30 verified websites, 29 career pages, 27 verified job lists, and 23 exact openings; `.69` replay of the same snapshots reaches 28 job lists. A fresh Viking/Akkodis/Adobe focused live retained 3/3 official job lists with no hard timeout, and its complete replay passed 3/3 with zero fixture gaps or mismatches. Authenticated LinkedIn extension Scan/Run acceptance remains deferred while backend reliability work continues.
