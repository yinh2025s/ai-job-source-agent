...
Current adapter iteration: `2026-07-14.77`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. ADR-0013 makes compatible batch completions terminal only when they succeeded, explicitly failed non-retryably, or cannot be safely classified; an explicit retryable first failure is resubmitted after invalidating only that stage and its downstream checkpoints. The source completion remains the crash fallback until atomic replacement, and trace/summary expose privacy-safe restore/resubmit provenance. Career search now disables an explicitly non-retryable source for the rest of one resolver invocation, without affecting other sources or later companies. Request-aware snapshot replay also selects the highest-sequence page or failure for each exact request identity, so a stale timeout cannot mask a later successful response. The frozen-30 run recovered through two ordinary resumes from 30/28/27/22 to the published 30 websites, 29 career pages, 28 verified job lists, and 24 exact openings; the final six failure outcomes and all 30 outcomes replay with zero gaps or mismatches. Final gates pass 1081 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. Attempt/stage-scoped snapshot lineage remains the next persistence contract; ambiguous replay continues to fail closed. Authenticated LinkedIn extension Scan/Run acceptance remains deferred while backend reliability work continues.
