...
In `chrome://extensions`, enable Developer mode, choose **Load unpacked**, and select the repository's `extension/` directory. After updating the repository, use **Reload** on the extension card so Chrome loads manifest version `0.4.0`. Open the extension's **Connection** section, keep `http://127.0.0.1:8765`, enter the same token, and save. Only the exact loopback form `http://127.0.0.1:<port>` is accepted. On LinkedIn Jobs, **Scan selected** immediately shows the current job and any DOM-observed **LinkedIn Apply** URL. **Scan page** walks the currently loaded result batch, waits within a bounded budget for each matching detail's Apply state, reports progress, can be cancelled, and returns at most 30 job records before restoring the original selection. For a selected button-only posting, **Capture target** freezes its exact job identity; click LinkedIn's real company-website Apply button, open the extension on the resulting page, and use **Confirm target**. The target must be bound to the source tab and pass browser plus Python URL validation. Optional **Verify source** then submits the record to the strict pipeline; it is never started automatically. Closing and reopening the popup does not cancel a bridge-owned verification run. Results, trace, and summary files are stored under `~/.ai-job-source-agent/runs/<run-id>/`.
...
The extension never reads cookies or sends LinkedIn page data to a remote service. The bridge binds only to a loopback host, requires the bearer token, accepts Chrome-extension origins, limits request size and record count, and delegates all network/provider decisions to the Python application. DOM-visible External Apply links are optional evidence. Button-only targets require an explicit single-job user capture, session-only pending state, exact source/opener binding and double URL sanitization; the extension never clicks Apply automatically or observes arbitrary background tabs.
