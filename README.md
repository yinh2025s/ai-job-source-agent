...
Current adapter iteration: `2026-07-13.43`. Verified official-website evidence from a public LinkedIn company page can now be reused for 30 days when a later live request is throttled or incomplete. The resolver always tries live evidence first, identifies cache entries by normalized company name plus normalized LinkedIn company URL, and records whether accepted evidence came from `live` or `cache` in trace. Offline validation passes 616 tests, 21/21 provider cases, 6/6 resolver cases, and architecture validation with 19 adapters / 0 issues. A clean rerun of the same frozen 30-company cohort produced 29 websites, 23 verified job lists, and 18 exact openings; authenticated Chrome installation and one real LinkedIn DOM scan remain pending manual acceptance.
