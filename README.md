...
Current adapter iteration: `2026-07-14.72`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. S5 now stops discovery when first-party page evidence has already produced a typed listing-capable board; detection-only evidence still allows the bounded ATS fallback. The architecture gate reports and validates those capabilities separately. Final offline gates pass 1006 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. The latest frozen live run remains 30 verified websites, 29 career pages, 27 verified job lists, and 23 exact openings. A focused Akkodis hybrid run skipped blind ATS search, read all 83 Sitecore records across nine live API pages, and returned a verified no-match; the two full-network attempts timed out before that path and are not counted as full live success. Authenticated LinkedIn extension Scan/Run acceptance remains deferred while backend reliability work continues.
