...
Current adapter iteration: `2026-07-14.83`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. ADR-0015 keeps company URL/provider/tenant identity and authoritative checkpoint prefixes release-blocking. ADR-0016 now verifies preferred and LinkedIn-official website evidence before speculative domains, while retaining all existing identity, redirect, parking, and conflict checks; only an exact four-or-more-letter institutional acronym on `.edu` may use a 401/403 host response as constrained evidence. Job-link scoring and first-party portal promotion also share one bounded command taxonomy. A four-company focused live run recovered RIVR to its official Lever exact opening and Bosch to `jobs.bosch.com`; a clean Atira rerun reached its Ashby exact opening in 6.9 seconds; SNHU reached the correct `snhu.edu` website but remained an honest S4 failure because the site returned 403 and search produced no verified career page. The fixed production provider benchmark remains 25/25, and final offline gates pass 1226 tests, 6/6 resolver cases, and 24 adapters / 0 architecture issues. The prior strict 15-company baseline remains 6/15 pre-frozen exact opening identities until the same full cohort is rerun. Visa multi-board selection and Hostinger first-party dynamic inventory are the next separate contracts. Authenticated LinkedIn extension Scan/Run remains a manual acceptance gate; the user performs the Chrome clicks and the backend validates bridge artifacts and logs.
