...
Current adapter iteration: `2026-07-14.76`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. ADR-0012 lets S2 publish a privacy-bounded, URL-only `HomepageNavigationEvidence` value through the execution checkpoint; S4 still fetches and verifies every candidate, but can skip a duplicate homepage transport when the exact verified homepage matches. In the frozen-30 live run, 21/27 S4 executions consumed the handoff and total S4 dispatches fell from 107 to 70 (-34.6%), with no budget rejection. The raw serialized run recorded 30 websites, 29 career pages, 28 job lists, and 22 exact openings; Suffolk exposed a generic canonical-provider query bug and Adobe hit a transient Phenom timeout. After the generic fixes, a focused Suffolk/Adobe live run was 2/2 exact and 2/2 replay, while the full 30-company snapshot replay was 30/30 reproduced with zero gaps or mismatches. Final offline gates pass 1067 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. The last complete published full-cohort baseline remains 30/29/28/24. Authenticated LinkedIn extension Scan/Run acceptance remains deferred while backend reliability work continues.
