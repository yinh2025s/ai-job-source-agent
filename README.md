...
Current adapter iteration: `2026-07-14.70`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. Full-outcome replay fails closed on dropped records, and evaluation baselines bind the observed result count. S5 preserves retryable evidence-backed failures instead of reporting a deterministic missing board; a previously verified career root remains tier-0 evidence when its S5 refetch times out. Phenom derives a customer-owned search board only from validated tenant/CDN/base-route state. Career search and Sitecore pagination stop cooperatively before publication time is consumed, native budget interruption does not trigger generic fallback, and parent timeout recovery publishes durable-stage provenance. Final offline gates pass 1000 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. The `.69` frozen live run produced 30 verified websites, 29 career pages, 27 verified job lists, and 23 exact openings with no Akkodis hard timeout. Adobe's sole deterministic S5 miss replays as retryable `NETWORK_TIMEOUT` with zero fixture gaps under `.70`. Authenticated LinkedIn extension Scan/Run acceptance remains deferred while backend reliability work continues.
