...
Current adapter iteration: `2026-07-14.59`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. ADR-0005/0006 define typed board handoff and request-aware snapshot replay; ADR-0007 binds agent/batch execution configuration to checkpoints, completions, baselines, and replay; ADR-0008 makes the company deadline a durable-publication boundary; ADR-0009 distinguishes audited budget recovery from replay drift. Budget workers run in an isolated process group, publish large results through an atomic local envelope, and terminate browser descendants on timeout. Snapshot content is durable before its JSONL index, while valid stage checkpoints remain reusable and company completion stays the final commit marker. Final gates pass 891 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. The frozen 30-company capability result remains 30 verified websites, 28 career pages, 26 verified job lists, and 23 exact openings pending the next full-cohort run. WhiteCarrot supports both anonymous tenant inventory and provider-hosted SSR boards; a two-tenant focused live and its two-fixture replay pass 2/2 exact openings. Authenticated LinkedIn extension Scan/Run acceptance is deferred by the current work plan.
