...
Current adapter iteration: `2026-07-14.51`. The registry auto-discovers 23 adapter modules: 21 provide native inventory or constrained positive evidence, while CEIPAL and Talemetry are detection-only and never invent candidates or no-match. ADR-0005 adds a typed S5→S6 board handoff; checkpoint schema 1.2 accepts only seven registered public-locator policies and rejects unknown providers, cross-origin evidence, sensitive query keys, credential/HTML-shaped content, and oversized values. Runtime-only identities such as CEIPAL's API-key-shaped widget value are omitted. First-party JS navigation recovers Direct Supply's labeled same-origin career route and current Workday exact opening. Failure replay compares outcome signatures, removes managed stale fixtures/checkpoints on output reuse, and exits nonzero for mismatches or fixture gaps; summaries rank actionable `stage x provider x reason` clusters. Final CPython 3.12 gates pass 774 tests, 23/23 provider cases, 6/6 resolver cases, and 23 adapters / 0 architecture issues. The serialized frozen-30 live cohort is 30 websites, 28 career pages, 26 verified job lists, and 20 exact openings; its 10 non-success replays classify as 6 reproduced, 2 fixture gaps, and 2 mismatches. Authenticated LinkedIn extension Scan/Run acceptance remains deferred.
