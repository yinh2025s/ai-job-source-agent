...
Current adapter iteration: `2026-07-14.82`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. ADR-0015 makes company-level website, career, provider, public tenant, board, and opening identity part of the release gate; expectations may freeze a verified subset, but every declared field is strict and baseline reports show field-level drift. CLI, live resume, automatic retry, explicit rerun, and timeout recovery share one contiguous authoritative checkpoint-prefix policy. The fixed production provider benchmark remains 25/25. A new 15-company unfamiliar holdout reached 13 websites, 12 career pages, 8 verified job lists, and 7 title-matched openings, but only 6 openings matched the URLs frozen before execution. Two cross-company false positives found in the first run were reduced to zero rather than counted as successes. Final offline gates pass 1217 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. Authenticated LinkedIn extension Scan/Run remains a separate manual acceptance gate; the user performs the Chrome clicks and the backend validates bridge artifacts and logs.
