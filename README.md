...
In `chrome://extensions`, enable Developer mode, choose **Load unpacked**, and select the repository's `extension/` directory. After updating the repository, use **Reload** on the extension card so Chrome loads manifest version `0.2.0`. Open the extension's **Connection** section, keep `http://127.0.0.1:8765`, enter the same token, and save. Only the exact loopback form `http://127.0.0.1:<port>` is accepted. On a LinkedIn Jobs detail or search-results page, **Scan page** collects up to 30 visible records; **Run discovery** submits an asynchronous run and reports verified job-list and exact-opening rates. Closing and reopening the popup does not cancel a bridge-owned run. Results, trace, and summary files are stored under `~/.ai-job-source-agent/runs/<run-id>/`.
