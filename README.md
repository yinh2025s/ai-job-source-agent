...
Current adapter iteration: `2026-07-14.85`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. ADR-0018 adds bounded first-party dynamic inventory: S5 follows at most three same-origin page assets and one static client dependency, accepts only a literal public job-list GET, validates one complete native-provider tenant, and never reads bundle credentials. Same-origin `api-proxy` calls synthesize only the public page-host marker, so sanitized scoped replay remains deterministic. S6 now combines title score with ordered title identity, preventing high-overlap role-family false positives. Hostinger focused live verifies 77 current first-party/Ashby records and reaches its official board, then correctly returns verified no-match because the frozen `AI Engineer` opening is no longer public; fresh scoped replay reproduces that outcome. Final gates pass 1266 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 issues. The prior strict 15-company baseline remains 6/15 pre-frozen exact opening identities until the same full cohort is rerun. Authenticated LinkedIn extension Scan/Run remains a manual acceptance gate; the user performs the Chrome clicks and the backend validates bridge artifacts and logs.
