...
Current adapter iteration: `2026-07-14.73`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. S5 validates strong safe same-site listing routes before speculative provider assets or page probes, while visible canonical provider boards and typed page evidence retain precedence. The per-process page cache now treats the two spellings of an HTTP(S) origin root as one internal alias without changing transport or snapshot identity. Results-only and trace-based failure replay both rebuild page-derived boards from sanitized S5 evidence rather than reconstructing provider locators from diagnostics. Final offline gates pass 1020 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. The fresh serialized frozen-30 live run reaches 30 verified websites, 29 career pages, 28 verified job lists, and 24 exact openings; full-outcome replay is 30/30 with zero fixture gaps or mismatches. Akkodis reduced S5 to 55 ms but read 70/83 Sitecore records after a slow S4, so it remains an honest cooperative-budget partial rather than a claimed verified no-match. Authenticated LinkedIn extension Scan/Run acceptance remains deferred while backend reliability work continues.
