...
Current adapter iteration: `2026-07-14.67`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. Replay restores verified response URL/final-URL identity, preserves typed fixture gaps, and compares provider-declared canonical board identity. Website resolution now requires positive company identity rather than treating HTTP success or a historical slug as proof; S4 can recover labeled same-site career anchors from bounded first-party bundles and can verify generic cross-domain career redirects only through a strict identity, canonical URL, actionable route, and corporate-backlink chain. Final gates pass 967 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. The latest frozen 30-company run remains 30 verified websites, 29 career pages, 28 verified job lists, and 24 exact openings. A focused live run moved Stash, Airbnb, Peloton Interactive, and Solomon Page from zero career pages to four, with Airbnb also reaching its verified positions list; all four outcomes replayed with zero gaps or mismatches. Authenticated LinkedIn extension Scan/Run acceptance remains deferred while backend reliability work continues.
