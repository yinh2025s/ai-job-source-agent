...
Current adapter iteration: `2026-07-14.79`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. S5 now receives the target location, prefers matching regional boards, permits region-neutral fallback, and excludes explicit conflicts before fetch or provider handoff; a matching-board timeout remains retryable instead of silently falling through to another country. Sitecore Next accepts either the page-bound primary brand or its dictionary brand, compares only the language primary subtag, and keeps country, tenant, ID, duplicate, and mixed-page checks strict. A focused Akkodis live run selected the U.S. board, excluded the Belgian board, read all 83 official records across nine pages, and returned verified `OPENING_NOT_FOUND` for the stale LinkedIn title in 45.2 seconds. Its fresh scoped bundle v6 replayed 1/1 with zero gaps or mismatches. Scoped replay also preserves validated CRLF page bytes, preventing universal-newline conversion from creating false digest corruption. The published frozen-30 baseline remains 30 websites, 29 career pages, 28 verified job lists, and 24 exact openings; a focused result is not used to recalculate that cohort. Final gates pass 1148 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. Authenticated LinkedIn extension Scan/Run acceptance remains deferred while backend reliability work continues.
