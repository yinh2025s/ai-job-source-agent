...
Current adapter iteration: `2026-07-13.44`. Public LinkedIn search cards now declare only `listed + unknown`, while an authenticated detail DOM can declare native/external apply or an explicit closed state only from visible controls. When official career/ATS discovery is deterministically exhausted, trusted active LinkedIn-native evidence becomes `partial / LINKEDIN_NATIVE_ONLY` without creating a company job-list or opening URL; network and incomplete discovery failures retain precedence. Source semantics participate in checkpoint fingerprints without volatile timestamps, and evaluation reports source dispositions separately from official provider-inventory diagnostics. Offline validation passes 640 tests, 21/21 provider cases, 6/6 resolver cases, and architecture validation with 19 adapters / 0 issues. The `.43` clean frozen 30-company baseline remains 29 websites, 23 verified job lists, and 18 exact openings; the unpacked extension is installed, while one authenticated scan/result acceptance remains pending.
