...
Current adapter iteration: `2026-07-14.81`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. The fixed provider release benchmark now executes the production `PipelineApplication` and full S1-S7 composition instead of the legacy direct-agent boundary; all 25 cases retain exact openings, complete lineage, and one run-configuration identity. Evaluation reports one durable terminal semantic per company, separating exact openings, verified no-match, no public openings, identity ambiguity, retryable failures, external blocks, unsupported capability, replay infrastructure failure, and unresolved discovery without scanning intermediate trace errors. Legacy snapshot materialization rejects any schema-v3 scoped record before touching output and directs callers to bundle-v6 replay, so a modern capture cannot become an empty legacy success. The fresh frozen-30 run reached 30 websites, 29 career pages, 27 verified job lists, and 24 exact openings; typed completion resume recovered Akkodis to 28 job lists while Percepta remained an explicit Taleo/network retryable. The final semantic distribution is 24 exact, 3 verified no-match, 1 no-public-openings, 1 identity-ambiguous, and 1 retryable; full scoped replay reproduced 30/30 with zero gaps or mismatches. Final offline gates pass 1168 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. Authenticated LinkedIn extension Scan/Run acceptance remains explicitly deferred while backend reliability work continues.
...
Verified LinkedIn official-website evidence is stored locally as a small company-level cache. Pass `--linkedin-evidence-cache PATH` to choose its location; otherwise checkpointed CLI runs use `<checkpoint_dir>/linkedin-website-evidence.json`, and extension runs reuse `~/.ai-job-source-agent/runs/linkedin-website-evidence.json`. Reads and read-modify-write saves use a process lock, and writes publish with a same-directory temporary file plus atomic replace. Missing, corrupt, schema-mismatched, malformed, future-dated, or non-finite records are safe cache misses; the default TTL is 30 days.

The cache contains only normalized company names, LinkedIn company URLs, public official-website URLs, and observation timestamps. It never stores personal profiles, job-page HTML, cookies, tokens, headers, browser storage, or authenticated LinkedIn payloads, and company-specific cache files must not be committed. Cached evidence is not an identity override or a successful result: it still passes the normal redirect, parking, region, and brand checks, and it cannot by itself produce a career page, job list, or opening.
...
- July 13 replay website revalidation: historical `replay_input` websites are verified as preferred candidates instead of being trusted indefinitely. Sedo and Squarespace parking infrastructure is rejected before resolver selection, while ordinary direct inputs retain their no-fetch path. Suffolk Construction migrates from two parked domains to `suffolk.com`, follows the official iCIMS board, and resolves the exact Site AI Engineer opening. The frozen 11-company failure cohort improves from 0/11 to 1/11 exact without promoting the other ten failures; the `.37` gates pass 560 tests, 20/20 provider cases, 6/6 resolver cases, and the 18-adapter architecture validator.
- July 13 startup team routes and Magnolia delivery: an official homepage `/team` link is considered only when the destination has strong employment or ATS evidence. A bounded first-party content probe can also enrich a JavaScript shell from its declared public Magnolia Delivery payload while enforcing same-site module, brand-aligned HTTPS CMS host, and same-host response checks. GPTZero reaches its current Ashby board and correctly reports the stale target title absent; Direct Supply reaches Workday and the exact `AI Engineer` `REQ-2026-2441`. Both sanitized captures replay in under 0.3 seconds. The frozen 11-company cluster is now 3/11 job lists and 2/11 exact with no new false positives; the `.38` gates pass 563 tests, 20/20 provider cases, 6/6 resolver cases, and the 18-adapter architecture validator.
...
- July 13 LinkedIn website-evidence cache: ADR-0003 adds a 30-day, process-safe cache keyed by normalized company plus LinkedIn company URL. S2 tries live evidence before cache fallback, retries the LinkedIn company URL with a trailing slash when needed, and labels trace provenance as `live` or `cache`; CLI, checkpoint-derived, and extension paths are stable. The content script now excludes hidden DOM records without clipping offscreen job cards, and real loopback handler tests cover CORS plus HTTP 401/403/404/413. The `.43` gates pass 616 tests, 21/21 provider cases, 6/6 resolver cases, and architecture validation with 19 adapters / 0 issues. A clean rerun of the exact frozen 30-company manifest improved the baseline from 27/17/14/11 to 29 websites, 23 career pages, 23 verified job lists, and 18 exact openings; the clean cache recorded four live evidence observations and no cache fallback, so this result does not use seeded records. Authenticated Chrome installation plus one real LinkedIn DOM scan remain pending manual acceptance. Release quality is judged by unfamiliar frozen samples, not cache-hit count.
- July 14 CEIPAL public inventory and typed failure semantics: a first-party CEIPAL widget is verified through its tenant-bound iframe and bounded public multipart inventory without persisting the credential-bearing path or form values. Centraprise resolves the exact first-party opening live and from 29 deterministic fixtures. Explicit visible empty inventory, an undisclosed recruiting-agency client, and exhausted candidate-fetch budget now produce separate typed outcomes. The `.53` gates pass 817 tests, 24/24 provider cases, 6/6 resolver cases, and 23 adapters / 0 architecture issues. The same serialized frozen-30 cohort reaches 30 websites, 28 career pages, 27 verified job lists, and 22 exact openings; all eight non-success outcomes replay from 291 fixtures with zero gaps or mismatches. Authenticated LinkedIn extension Scan/Run remains deferred.
- July 14 WhiteCarrot public inventory: the native adapter recognizes both `app.whitecarrot.io/(share/)careers/{tenant}` and provider-owned `*.whitecarrot.ai/jobs` boards. Tenant mode reads the anonymous one-shot careers API; custom-host mode parses only strong same-origin Next SSR job items. Both modes reject credentials, nonstandard ports, cross-origin or mismatched UUID details, profile-builder talent pools, malformed inventory, and unsafe redirects. Smart Bricks and Whitecarrot pass focused live 2/2 exact, and two sanitized snapshots replay 2/2 with no privacy exclusions or fixture gaps. The `.59` gates pass 891 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues.
- July 14 canonical provider-board handoff: visible provider links are handed off directly when the linked URL already equals the adapter's canonical listing root. This removes dependence on the generic opaque-path detail heuristic while detail and legacy URLs still fail canonical equality and retain their fetch/redirect checks. The frozen `.59` cohort reaches 30/28/27/23 and exposes the Smart Bricks handoff miss; `.60` then resolves Smart Bricks through the WhiteCarrot public API to the exact AI Engineer opening in 9.7 seconds. Six sanitized records become five fixtures with no privacy exclusions and replay exact in 0.5 seconds. The `.60` gates pass 892 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues.
- July 14 page-derived failure replay: an S6 failure whose S5 board required public page evidence now reruns S5 from sanitized snapshots, preserving ADR-0005's rule that diagnostic trace is not a runtime locator API. URL-native providers retain the shorter S6 resume path. Replaying the `.59` failure capture improves from five reproduced outcomes and two fixture gaps to six reproduced and one honest Akkodis timeout gap; Viking again reaches its verified Phenom title-filtered no-match. The `.61` gates pass 894 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues.
- July 14 hard-timeout checkpoint recovery: the live parent reconstructs only a contiguous, compatible, durably published stage prefix after terminating a timed-out or failed child. Completed website, career, board, provider, and trace evidence survive; the first missing stage receives the structured failure, while corrupt, incompatible, semantically invalid, or missing checkpoints block all later recovery and fields. Akkodis completes its nine-page, 83-record Sitecore inventory in 43.6 seconds and honestly reports the stale title absent; 16 fixtures replay the same outcome with no privacy exclusions or gaps. The `.62` gates pass 900 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues.
