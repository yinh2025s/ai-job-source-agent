...
Current adapter iteration: `2026-07-14.78`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. ADR-0014 binds every captured terminal fetch outcome to an opaque invocation ID, execution fingerprint, and canonical stage. Durable stage checkpoints and batch completions retain typed lineage, so a selective retry can safely combine restored upstream stages with a newly computed suffix without searching a shared snapshot root for plausible evidence. Snapshot v3 records materialize as strict per-stage outcome tapes; replay bundle v6 gives every source occurrence an isolated record ID, checkpoint root, application, cache, and tape cursor. Missing, extra, corrupt, cross-stage, or unconsumed outcomes fail closed, while legacy v1/v2 captures remain available only through explicit bundle v5 materialization. A real `SIGKILL` acceptance verifies that evidence written by an interrupted S5 remains orphaned, the resumed S5-S7 use a new attempt, and only checkpoint-referenced scopes enter replay. The published frozen-30 product baseline remains 30 websites, 29 career pages, 28 verified job lists, and 24 exact openings; this persistence iteration does not claim a live-rate increase. Final gates pass 1138 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. Authenticated LinkedIn extension Scan/Run acceptance remains deferred while backend reliability work continues.
