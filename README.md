...
Current adapter iteration: `2026-07-14.57`. The registry auto-discovers 23 adapter modules: 22 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. ADR-0005/0006 define typed board handoff and request-aware snapshot replay; ADR-0007 binds agent/batch execution configuration to checkpoints, completions, baselines, and replay; ADR-0008 makes the company deadline a durable-publication boundary. Budget workers run in an isolated process group, publish large results through an atomic local envelope, and terminate browser descendants on timeout. Snapshot content is durable before its JSONL index, while valid stage checkpoints remain reusable and company completion stays the final commit marker. Final gates pass 871 tests, 24/24 provider cases, 6/6 resolver cases, and 23 adapters / 0 architecture issues. The same frozen 30-company cohort now reaches 30 verified websites, 28 career pages, 26 verified job lists, and 23 exact openings. Complete native inventory is terminal for a verified no-match, which lets Akkodis publish its current Sitecore result within the 45-second budget instead of spending time on redundant HTML fallbacks. Authenticated LinkedIn extension Scan/Run acceptance remains deferred.
