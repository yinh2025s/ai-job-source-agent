...
Current adapter iteration: `2026-07-14.80`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. S5 may inspect the initial verified career root as a cross-region navigation hub, but every promoted provider board, job list, and opening still passes the target-region eligibility gate. This recovered Dematic's U.S. Workday board from its official Australia career landing page without accepting the Australia board as the result. Scoped bundle v6 replay now reconstructs the original source and input semantics from authoritative S1-S4 evidence, and its outcome tape matches exact request identities while preserving occurrence order for duplicates; independent concurrent requests may replay in a different scheduling order, but missing, extra, mismatched, or unconsumed outcomes still fail closed. The fresh frozen-30 run reached 30 websites, 29 career pages, 27 verified job lists, and 23 exact openings; full scoped replay reproduced 30/30 with zero gaps or mismatches. A same-config Dematic rerun recovered its U.S. job list and exact AI Application Engineer opening and replayed 1/1, so the initial timeout remains classified as transient rather than hidden by a cohort edit. Final offline gates pass 1159 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. Authenticated LinkedIn extension Scan/Run acceptance remains explicitly deferred while backend reliability work continues.
