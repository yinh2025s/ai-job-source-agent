...
Current adapter iteration: `2026-07-13.45`. Career discovery now verifies primary homepage evidence before sitemap fanout, rejects resource and unrelated cross-site sitemap noise, and uses the posting location to prioritize and stop on matching regional sitemaps. Missing guessed ATS tenants return a non-retryable `HTTP_NOT_FOUND`, LinkedIn official-site evidence can reuse same-domain verification work, and Avature normalizes Unicode dashes only for search queries. Offline validation passes 647 tests, 21/21 provider cases, 6/6 resolver cases, and architecture validation with 19 adapters / 0 issues. The frozen five-company regression run reached 5/5 websites, careers, and verified job lists plus 4/5 exact openings; Centraprise is a verified inventory no-match, and Deloitte also replays its exact Avature opening from the frozen v33 snapshot. The `.43` clean frozen 30-company baseline remains 29 websites, 23 verified job lists, and 18 exact openings; the unpacked extension is installed, while one authenticated scan/result acceptance remains deferred.
