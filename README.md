...
Current adapter iteration: `2026-07-14.68`. The registry auto-discovers 24 adapter modules: 23 provide native inventory or constrained positive evidence, while only Talemetry remains detection-only. Supplied websites now pass through the same bounded, identity-verifying resolver used by replay; verified staffing semantics can trigger a public LinkedIn posting-identity probe without deciding the employer on website evidence alone. S5 preserves typed provider handoffs across redirects back to first-party career roots and traverses only explicit same-site `all-jobs` commands; S6 accepts a strict same-origin numeric-detail route contract. Final gates pass 983 tests, 25/25 provider cases, 6/6 resolver cases, and 24 adapters / 0 architecture issues. The latest frozen 30-company run remains 30 verified websites, 29 career pages, 28 verified job lists, and 24 exact openings. Focused live now resolves Stash through Greenhouse to an exact opening and migrates Peloton's stale supplied domain to `onepeloton.com` before finding its exact Product Manager opening; the Peloton full-outcome replay is 1/1 reproduced with zero gaps or mismatches. Solomon Page correctly remains publisher-unconfirmed and does not promote customer staffing inventory into company-owned openings. Authenticated LinkedIn extension Scan/Run acceptance remains deferred while backend reliability work continues.
