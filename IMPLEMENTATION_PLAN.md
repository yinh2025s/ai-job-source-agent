...
- provider-specific ATS 能力已迁移到自动发现的独立 registry/adapter modules，共 19 个原生 adapter
...
> 七关状态模型、统一错误码、benchmark 矩阵和 SOLID 并行开发架构已完成第一版。S1-S7 都有独立 stage class，19 个主要 provider（含 Google Careers、Phenom、Paycom、RippleHire、Taleo、Eightfold、JazzHR、Avature 和 Breezy）已迁移到自动发现的原生 adapter，通用 ApplicationRunner、并发安全 filesystem stage store 和原子 company completion store 已接管 production CLI 与 live batch。Manifest V3 本地扩展可从登录态 LinkedIn Jobs 页面采集可见 evidence，经 token-protected loopback bridge 复用同一 pipeline；受信任浏览器/页面输入还可提供 `external_apply_url`，在官网失败时经 S5 native-provider gate 和 S6 真实库存验证继续发现岗位，公开 guest LinkedIn 不承诺提供该字段。严格名称匹配的公开 LinkedIn 官网 evidence 现在可在 30 天 TTL 内按规范化 company + LinkedIn company URL 复用；resolver 始终 live first，cache fallback 带明确 trace provenance，缓存仍需通过统一官网校验且不含个人、认证或浏览器会话数据。失败样本会由内容寻址 snapshot 自动生成离线 replay bundle，query pagination 与 redirect alias 已可确定性回放；S3 对投资/招聘中介发布者执行有界 posting-identity probe，区分实际雇主与未披露客户；first-party JS shell 可通过严格来源约束的公开 CMS 或 module asset 补充招聘证据。固定 provider benchmark 为 21/21 exact opening，resolver benchmark 为 6/6；CPython 3.12 下 `.43` 通过 616 tests 与 architecture validation 19 adapters / 0 issues，3.14 暂不支持。相同冻结 30-company clean run 为 29/30 官网、23/30 verified job list、18/30 exact opening，且未读取 seeded cache；Chrome `Load unpacked` 与一次登录态 LinkedIn DOM scan 仍待人工验收。最终 release 指标是陌生冻结样本结果，不是 cache hit、并行任务数或已知 focused 样本；Meta Careers、CEIPAL 与 generic fallback 仍是 unsupported/compatibility 后续路径。
