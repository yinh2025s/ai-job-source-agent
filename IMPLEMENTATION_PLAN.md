...
> 七关状态模型、统一错误码、benchmark 矩阵和 SOLID 并行开发架构已完成第一版。S1-S7 都有独立 stage class，19 个主要 provider（含 Google Careers、Phenom、Paycom、RippleHire、Taleo、Eightfold、JazzHR、Avature 和 Breezy）已迁移到自动发现的原生 adapter，通用 ApplicationRunner、并发安全 filesystem stage store 和原子 company completion store 已接管 production CLI 与 live batch。Manifest V3 本地扩展可从登录态 LinkedIn Jobs 页面采集可见 evidence，经 token-protected loopback bridge 复用同一 pipeline；受信任浏览器/页面输入还可提供 `external_apply_url`，在官网失败时经 S5 native-provider gate 和 S6 真实库存验证继续发现岗位，公开 guest LinkedIn 不承诺提供该字段。失败样本会由内容寻址 snapshot 自动生成离线 replay bundle，query pagination 与 redirect alias 已可确定性回放；历史 replay 官网会先复核，并可使用公司名匹配的 LinkedIn JSON-LD 官方官网字段迁移陈旧同名域名；S3 对投资/招聘中介发布者执行有界 posting-identity probe，区分实际雇主与未披露客户；first-party JS shell 可通过严格来源约束的公开 CMS 或 module asset 补充招聘证据。SmartRecruiters derived board 还能使用 adapter 产出的强租户身份验证，不要求旧职位标题仍然开放。固定 provider benchmark 为 21/21 exact opening，resolver benchmark 为 6/6；CPython 3.12 release baseline 下 587 tests 和最新 fixed live 51/51 官网与 job list、50/51 exact opening、51/51 expectations 通过；focused startup cluster 为 3/3 exact；3.14 暂不支持。5-provider/5-technology strict browser saved/live gate 为 5/5。Greenhouse、Lever、Ashby、Workday、SmartRecruiters、Workable、Rippling、BambooHR、iCIMS 和 SuccessFactors 各有 5 家固定 live 公司；Meta Careers、CEIPAL 与 generic fallback 仍是 unsupported/compatibility 后续路径。
